...
class TextToTopic(TextToX):
    def __init__(self,
        ...
        self.tokenlists = [self.__extract_tok(df, self.tokentype,
                                              self.pos_tags)
                           for df in self.ttt.get_token_dfs()]
        if lowercase:
            self.tokenlists = [[t.lower() for t in tl]
                               for tl in self.tokenlists]
        if stopword_removal == "nltk":
            self.__remove_stopwords_ntlk()
        else:
            raise ValueError(f"The method {stopword_removal} for removing stopwords is \
    ...
    def __remove_stopwords_ntlk(self):
        if isinstance(self.lang, str):
            if self.lang in nltk_lang_dict:
                ...
                ValueError(f"The language code {self.lang} is not valid in \
                             NLTK")
            stop_words = set(stopwords.words(lang))
            self.tokenlists = [[t for t in tl if t not in stop_words]
                               for tl in self.tokenlists]
        else:
            res = []
    ...
    def train_topic(self,
        ...
        if bad_tokens:
            dictionary.filter_tokens(bad_ids=[dictionary.id2token[tok] for tok
                                              in bad_tokens])

        self.bows = bows
        self.dictionary = dictionary
        if model == 'ldamulticore':
            self.model = models.LdaMulticore(bows,
                                             num_topics=num_topics,
                                             id2word=dictionary, **kwargs)
        if model == 'lda':
            self.model = models.LdaModel(bows,
                                         num_topics=num_topics,
                                         id2word=dictionary, **kwargs)
