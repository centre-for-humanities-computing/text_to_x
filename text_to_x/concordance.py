...
def extract_concordance(preprocessed_texts, tokens, type_token = "token", 
    ...
    def extract_concordance_single(df):
        ...
        def create_concordance(tok):
            concordance_sentences = df_matches[df_matches[tmp_token_name].isin(tokens)][sentence_id].unique()
            df_token_sentences = df[df[sentence_id].isin(concordance_sentences)]
            concordances = df_token_sentences.groupby([sentence_id])[type_token].apply(list).reset_index()
            concordances.columns = [sentence_id, "sentence"]
            concordances[type_token] = tok
            return concordances

        concordances_df = pd.concat([create_concordance(tok) for tok in tokens])
        concordances_df = concordances_df[[sentence_id, type_token, "sentence"]]
