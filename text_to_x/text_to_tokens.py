...
class TextToTokens(TextToX):
    def __init__(self, lang=None,
        ...
        detect_lang_fun (which defaults to polyglot). Can be a list of codes.
        ...
        args (list): can include "tokenize", "mwt", "lemma", "pos", "depparse",
        "stem"
        """
        ...
        if isinstance(method, str):
            ...
            raise TypeError(f"method should be a str or callable not a type:\
                            {type(method)}")
    ...
    def __stem(self, stemmer="snowball", **kwargs):
        if isinstance(self.lang, str):
            ...
            for i, la in enumerate(self.lang):
                if i == 0:
                    lang = la
                    self.__get_stemmer(stemmer, lang)
                elif la != lang:
                    self.__get_stemmer(stemmer, lang)
                self.__dfs[i]['stem'] = [self.stemmer(token) for token in
                                         self.__dfs[i]['token']]
